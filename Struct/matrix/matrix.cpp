...
Matrix<T> &Matrix<T>::operator=(const Matrix<T> &x) {
    if (&x != this) {
        ...
        for (int i = 1; i != x.m_rows + 1; ++i) {
            copy(x.m_element[i], x.m_element[i] + x.m_cols + 1, m_element[i]);
        }
    }
    return *this;
}
...
Matrix<T> &Matrix<T>::operator+(const Matrix<T> &x) const {
    if (x.m_rows != m_rows || x.m_cols != m_cols) {
        exit(-1);
    }
    static Matrix<T> add_m(m_rows, m_cols);
    for (int i = 1; i != m_rows + 1; ++i) {
        for (int j = 1; j != m_cols + 1; ++j) {
            add_m.m_element[i][j] = m_element[i][j] + x.m_element[i][j];
        }
    }
    return add_m;
}
