...
template <typename T> Linear_list<T>::Linear_list(int initial_capacity) {
...
template <typename T> Linear_list<T>::Linear_list(const Linear_list<T> &x) {
...
template <typename T> void Linear_list<T>::check_index(int the_index) const {
...
template <typename T> int Linear_list<T>::indexOf(const T &e, int pos) const {
  int index;
...
template <typename T> const T &Linear_list<T>::erase(int the_index) {
  check_index(the_index);
...
template <typename T> void Linear_list<T>::output() const {
  //    std::cout;
...
template <typename T> void Linear_list<T>::trim_to_size() {
  if (list_size > 1) {
...
template <typename T> int Linear_list<T>::set_size(int size) {
  if (size < 0) {
    exit(-1);
  }
...
template <typename T> T &Linear_list<T>::operator[](int i) const {
  check_index(i);
  return element[i];
}

template <typename T> bool Linear_list<T>::operator==(Linear_list<T> &x) const {
...
template<typename T>
void Linear_list<T>::push_back(T &e) {

}
template<typename T>
void Linear_list<T>::pop_back() {

}
template<typename T>
void Linear_list<T>::leftShift(int the_shift) {

}
template<typename T>
void Linear_list<T>::reverse() {

}
template<typename T>
Linear_list<T> &Linear_list<T>::meld(Linear_list<T> &x) {
  return <#initializer#>;
}
template<typename T>
Linear_list<T> &Linear_list<T>::merge(Linear_list<T> &) {
  return <#initializer#>;
}